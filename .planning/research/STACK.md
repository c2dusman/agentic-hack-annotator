...
**Domain:** Automated URL-to-annotated-image pipeline (screenshot capture + AI vision + AI copywriting + image rendering)
**Researched:** 2026-04-04
**Confidence:** HIGH (all core choices verified against official docs and npm)
...
pm2 startup   # generates systemd hook for auto-start on reboot
```
...
**For the output renderer (HTML template to PNG):**
...
**For Gemini structured output (vision analysis JSON):**
...
**If concurrent requests become a problem (post-hackathon):**
...
Node 20 reaches end-of-life this month (April 2026). The VPS is pre-provisioned with Node 20 per the project spec, so use it for the hackathon. Post-hackathon: upgrade to Node 22 LTS.

### 2. Express 5 is now the npm default — not Express 4

Running `npm install express` without a version pin now installs Express 5.1.x. The project brief says "Express 4.x" but since this is greenfield, using 5.x costs nothing and gives a longer support window. The only breaking change relevant here is the `path-to-regexp` syntax change — avoid using regex patterns in route paths (you likely aren't).

### 3. Two different Google AI SDKs exist — use the new one

`@google/generative-ai` (old, frozen at 0.24.x) vs `@google/genai` (new, 1.48.x, actively maintained). The old package does not support Gemini 2.5 structured outputs properly. Import path: `import { GoogleGenAI } from '@google/genai'`.

### 4. Puppeteer headless string `'new'` is deprecated

Use `headless: true` (boolean). The `'new'` string was a transitional option; since v22 `true` IS the new headless. Passing the string produces deprecation warnings in v24.

### 5. --disable-dev-shm-usage is mandatory on VPS

Docker containers and many VPS setups have `/dev/shm` capped at 64MB. Chrome crashes when it exhausts shared memory. Always include `--disable-dev-shm-usage` in launch args even if the VPS isn't a container — it makes Chrome write to `/tmp` instead.

### 6. Verify Claude model string before coding

The model ID for Sonnet 4 variants changes as Anthropic releases updates. At research time the identifier is `claude-sonnet-4-5`. Always check `platform.claude.ai/docs/en/about-claude/models/overview` and use the exact string from the "API name" column. Hardcoding a stale model string causes silent API errors.
...
---
*Stack research for: AnnotatorAI — automated screenshot annotation pipeline*
*Researched: 2026-04-04*
